...
As of v1.10.0, the server supports multi-instance operation — if port 9323 is already in use, it automatically falls back through ports 9224–9232. The plugin scans all ports and connects to every active server.
...
The plugin will appear in your Development plugins list as "FigJam Desktop Bridge".
...
2. **Run the plugin:** Right-click → Plugins → Development → FigJam Desktop Bridge
...
1. MCP server starts WebSocket server on port 9323 (or next available port in range 9223–9232)
...
- Verify the MCP server is running (it starts the WebSocket server on port 9323)
- Check that the plugin is open in Figma — the WebSocket client is in the plugin UI
- Check the browser console (Plugins > Development > Open Console) for `[MCP Bridge] WebSocket connected to port 9323`
...
1. The MCP server tries port 9323 first. If it's taken, it falls back to 9224, 9225, etc. (up to 9232)
...
Without re-importing, the old plugin code only connects to port 9323. If your server fell back to a different port, the plugin won't find it.
